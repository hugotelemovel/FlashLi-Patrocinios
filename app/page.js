...
import { BarChart3, Users, Image as ImageIcon, Send } from 'lucide-react';
...
export default function App() {
  ...
  async function fetchEmpresas() {
    ...
    if (error) {
      console.error(error);
      showMessage(`Erro ao ligar à Base de Dados: ${error.message}`, 'error');
    } else if (data) {
      setEmpresas(data);
    }
    setLoading(false);
  ...
  async function addEmpresa(e) {
    ...
    if (error) {
      ...
      showMessage(`❌ ERRO: ${error.message} (Verifica se desativaste o RLS no Supabase!)`, 'error');
    } else if (data) {
  ...
  async function enviarProposta(empresa) {
    showMessage(`A enviar proposta rica para ${empresa.nome}...`, 'info');
    try {
      ...
      if (res.ok) {
        showMessage(`✅ Proposta enviada a ${empresa.nome}!`, 'success');
  ...
  async function enviarBroadcast() {
    ...
    try {
      ...
      if (res.ok) {
        showMessage('✅ Diário de Bordo enviado com sucesso!', 'success');
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ background: '#f8fafc', color: '#64748b', textAlign: 'left' }}>
                ...
                <th style={{ padding: '12px', borderBottom: '1px solid #e2e8f0', textAlign: 'right' }}>Ação</th>
              </tr>
            </thead>
            <tbody>
              {empresas.map(emp => (
